...
namespace Sylvester {
    ...
    internal class FrameClass {
        ...
        internal static FrameClass Empty = new FrameClass();    // The empty Expando class - all Expando objects start off w/ this class.
        ...
        /// the keys.
        /// </summary>
        internal FrameClass(string[] keys, int hashCode) {
            _hashCode = hashCode;
            _keys = keys;
        }
        ...
        /// </summary>
        internal FrameClass FindNewClass(string newKey) {
            ...
            lock (this) {
                ...
                for (int i = 0; i < infos.Count; i++) {
                    FrameClass klass = infos[i].Target as FrameClass;
                    if (klass == null) {
                        infos.RemoveAt(i);
                        i--;
                        continue;
                ...
                FrameClass ec = new FrameClass(keys, hashCode);
        ...
        /// </summary>
        internal int GetValueIndex(string name, bool caseInsensitive, Frame obj) {
            if (caseInsensitive) {
        ...
        /// </returns>
        private int GetValueIndexCaseInsensitive(string name, Frame obj) {
            int caseInsensitiveMatch = Frame.noMatch; //the location of the case-insensitive matching member
                ...
                for (int i = _keys.Length - 1; i >= 0; i--) {
                    if (string.Equals(
                        ...
                        if (!obj.IsDeletedMember(i)) {
                            if (caseInsensitiveMatch == Frame.noMatch) {
                                caseInsensitiveMatch = i;
                            } else {
                                //Ambigous match, stop searching
                                return Frame.ambiguousMatchFound;
                            }
                        }
                    }
                }
