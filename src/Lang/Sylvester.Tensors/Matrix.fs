...
type Matrix<'t, 'd10,'d9, 'd8, 'd7, 'd6, 'd5, 'd4, 'd3, 'd2, 'd1, 'e10,'e9, 'e8, 'e7, 'e6, 'e5, 'e4, 'e3, 'e2, 'e1 
    ...
    member inline x.Item(i:'i, j:'j) = 
        checkidx(i, x.Dim0)
        checkidx(j, x.Dim1)
        x._Array.[(int) i, (int) j]
    ...
    member inline x.For(start0, finish0, start1, finish1, f: int -> int -> 't -> unit) = 
    ...
    member inline x.ForAll(f: int -> int -> 't -> unit) = 
        for i in 0..(x.Dim0 - 1) do 
            for j in 0..(x.Dim1 - 1) do
                f i j x._Array.[i, j]
    ...
    static member inline (*) (l:Matrix<'t, 'ld10,'ld9, 'ld8, 'ld7, 'ld6, 'ld5, 'ld4, 'ld3, 'ld2, 'ld1, 'le10,'le9, 'le8, 'le7, 'le6, 'le5, 'le4, 'le3, 'le2, 'le1>, 
        ...
        let res = l._Matrix * r._Matrix
        l.Create(l.Dim0, r.Dim1, res.ToArray())
