...
type Continuous<'t when 't : equality>(map:Expr<'t->real> option, support:Set<real> option, pdf:Expr<real->real>, mean:Scalar<real> option) = 
    inherit RandomVariable<'t>(map, support, pdf |> integrate |> ProbabilityDensity |> Some, mean)
...
module ProbabilityDistribution =
    ...
    let uniform_continuous<'t when 't : equality> a b = Continuous<'t>(None, None, <@ fun x -> 1. / (b - a) @> , None)