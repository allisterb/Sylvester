...
type Matrix<'dim0, 'dim1, 't when 'dim0 :> Number and 'dim1 :> Number and 't:> ValueType and 't : struct and 't: (new: unit -> 't) and 't :> IEquatable<'t> and 't :> IComparable and 't :> IFormattable>
    ...
    member val Length = array.Length
    ...
    static member ofArray(data: 't array array) = let d = data |> Seq.map(fun a -> List.ofArray a) |> List.ofSeq in Matrix d

    static member (+)(l : Matrix<'n, 'm, 't>, r : Matrix<'m, 'p, 't>) : Matrix<'n, 'p, 't> = new Matrix<'n, 'p, 't>([])
