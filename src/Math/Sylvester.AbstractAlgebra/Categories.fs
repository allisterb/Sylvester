...
/// Map or function between objects of the same type.
type Map<'U when 'U: equality> = 'U -> 'U 
   
/// Morphism between 2 sets of the same type.
type Morph<'U when 'U: equality> = 
...
with       
    ...
    static member (*) (l:Morph<'U>, r:Morph<'U>) =
        ...
        | _ -> failwith "Only individual morphisms are composble."
        
    /// Identity morphism
    static member Id(s) = Morph(s, s, id)

type Morphisms<'n, 'U when 'n :> Number and 'U: equality> = Array<'n, Morph<'U>>

/// A collection of sets and collection of morphisms in some universe U.
type ICategory<'U, 'obn, 'mn when 'U: equality and 'obn :> Number and 'mn :> Number> = 
    abstract member Objects:Sets<'obn, 'U>
    abstract member Morphisms:Morphisms<'mn, 'U>

/// Base implementation of a category of sets and morphisms in some universe U, which is inherited by mathematical structures.
type Category<'U, 'obn, 'mn when 'U: equality and 'obn :> Number and 'mn :> Number>
