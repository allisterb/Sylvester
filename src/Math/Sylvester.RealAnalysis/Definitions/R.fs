...
module R =
    ...
    let sum expr x l u = Ops.Sum expr x (int_expr l) (int_expr u) |> ScalarTerm
    ...
    let lim f x v = Ops.Limit f x v |> ScalarTerm
       
    let lim_right f x v = Ops.LimitRight f x v |> ScalarTerm

    let lim_left f x v = Ops.LimitLeft f x v |> ScalarTerm

    let inline deriv_lim f x a = 
        lim <@ ((%f)(%x + %a) - (%f) %x) / %a @> a <@ 0. @>

    let diff (x:ScalarTerm<real>) (s:ISymbolic<_, real>) =  
        fail_if_not_var x
        fail_if_not_has_var (get_var x.Expr) s.Expr
        s.Mutate(Ops.Diff 1 x.Expr s.Expr)

    let integrate (x:ScalarTerm<real>) (s:ISymbolic<_, real>) = 
        fail_if_not_var x
        fail_if_not_has_var (get_var x.Expr) s.Expr
        s.Mutate(Ops.Integrate x.Expr s.Expr)

    let integrate_over (x:ScalarTerm<real>) l r (s:ISymbolic<_, real>) = 
        fail_if_not_var x
        fail_if_not_has_var (get_var x.Expr) s.Expr
        s.Mutate(Ops.DefiniteIntegral x.Expr (real_expr l) (real_expr r) s.Expr)

    let integrate_over_R (x:ScalarTerm<real>) f = integrate_over x minf'<real> inf'<real> f