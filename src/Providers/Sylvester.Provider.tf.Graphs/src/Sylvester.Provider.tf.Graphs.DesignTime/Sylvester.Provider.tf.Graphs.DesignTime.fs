module Sylvester.Provider.tf.GraphsImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Sylvester.Provider.tf.Graphs.DesignTime", "Sylvester.Provider.tf.Graphs.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Sylvester.Provider.tf.Graphs.DesignTime", "Sylvester.Provider.tf.Graphs.Runtime")])

    let ns = "Sylvester.Provider.tf.Graphs"
    let asm = Assembly.GetExecutingAssembly()
