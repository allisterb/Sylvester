...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("Sylvester.Provider.Collections.DesignTime", "Sylvester.Provider.Collections.Runtime")])

    let ns = "Sylvester.Provider.Collections"
    let asm = Assembly.GetExecutingAssembly()
