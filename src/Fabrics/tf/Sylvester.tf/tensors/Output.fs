...
[<AbstractClass>]
type Output(graph:IGraph, dt:TF_DataType, name:string, ?shape:int64[]) =
    inherit Tensor(graph, dt, name, defaultArg shape null)
    
    interface IGraphOutput with
        member x.Graph = graph
        member x.Name = name
        member x._Type = Convert.ToInt64(int dt) 
    
[<AbstractClass>]        
type Output<'d10, 'd9, 'd8, 'd7, 'd6, 'd5, 'd4, 'd3, 'd2, 'd1 when 'd10 :> Base10Digit and 'd9 :> Base10Digit 
...
and 'd1 :> Base10Digit>(graph:IGraph, dt:TF_DataType, name:string, shape:int64[]) =
    inherit Tensor(graph, dt, name, shape)
