...
type GraphTests() =
    ...
    let ``Can create graph`` () =
        ...
        defaultGraph <- g
        let m0 = Matrix<dim<100>, dim<60>>("m")
        let m1 = Matrix<dim<100>, dim<60>>("m")
    ...
    let ``Can create graph scope``() =
        ...
        let m2 = 
            use x = subscope "x"
            Matrix<dim<100>, dim<60>>("m")
        Assert.Equal("x/m_0", m2.Name)
        use x2 = subscope "x2"
        use x3 = subscope "x3"
        let m3 = Matrix<dim<20>, dim<5>>("m")
        Assert.Equal("x2/x3/m_0", m3.Name)
        ends x3
        let m4 = Matrix<dim<20>, dim<5>>("m")
        Assert.Equal("x2/m_0", m4.Name)
        ends x2
        let m5 = Matrix<dim<200>, dim<5>>("m")
